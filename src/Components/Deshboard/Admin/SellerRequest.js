    ...
    <div className="grid grid-cols-1 md:grid-cols-2 mt-[50px]">
      {sellerRequest?.length > 0 ? (
        sellerRequest?.map((seller) => (
          <div
            key={seller?._id}
            className="max-w-md p-8 sm:flex sm:space-x-6 m-2 bg-gradient-to-r from-purple-600 to-purple-400  text-gray-100 rounded-2xl"
          >
            <div className="flex flex-col space-y-4">
              ...
              <div className="space-y-1">
                ...
                <span className="flex items-center space-x-2">
                  ...
                  <span className=" ">{seller?.address}</span>
                </span>
              </div>
              <button
                onClick={() => handleApproval(seller)}
